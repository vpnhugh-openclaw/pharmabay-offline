...
const fmt = {
  price: (v: any) => v == null ? "—" : `$${Number(v).toFixed(2)}`,
...
export function ShopifyListings() {
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        const msg = `Pushed ${synced} product(s)${failed ? ` · ${failed} failed` : ""}${errors?.length ? ": " + errors.join("; ") : ""}`;
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const dlg = await window.electronAPI.shopifySaveDialog({
        title: "Export Listings CSV",
        defaultPath: "shopify_listings.csv",
  ...
  const handleBulk = async (patch: Record<string, any>, label: string) => {
    ...
    try {
      const ids = [...selected];
      const res = await window.electronAPI.shopifyBulkUpdateListings({ product_ids: ids, patch });
      if (res.error) { showStatus(res.error, false); return; }
          ...
          <table className="w-full text-xs border-collapse" style={{ minWidth: 900 }}>
            <thead className="sticky top-0 z-10 bg-muted/80 border-b backdrop-blur">
              <tr>
                ...
                <th className={thL} onClick={() => toggleSort("title")}><span className="inline-flex items-center gap-0.5">Title<SortIcon k="title" /></span></th>
                <th className={thL} onClick={() => toggleSort("vendor")}><span className="inline-flex items-center gap-0.5">Vendor<SortIcon k="vendor" /></span></th>
                <th className={thL} onClick={() => toggleSort("status")}><span className="inline-flex items-center gap-0.5">Status<SortIcon k="status" /></span></th>
                <th className={thR} onClick={() => toggleSort("_price")}><span className="inline-flex items-center justify-end gap-0.5">Price<SortIcon k="_price" /></span></th>
                <th className={thR}>Cost</th>
                <th className={thR}>GP%</th>
                <th className={thR} onClick={() => toggleSort("_stock_on_hand")}><span className="inline-flex items-center justify-end gap-0.5">SOH<SortIcon k="_stock_on_hand" /></span></th>
                <th className={thR} onClick={() => toggleSort("_units_sold_12m")}><span className="inline-flex items-center justify-end gap-0.5">Sold 12m<SortIcon k="_units_sold_12m" /></span></th>
                <th className={thS}>Barcode</th>
                <th className={thS}>Type</th>
                <th className={thS}>Tags</th>
                <th className={thS} onClick={() => toggleSort("has_edits")}><span className="inline-flex items-center gap-0.5">Edits<SortIcon k="has_edits" /></span></th>
            ...
            <tbody>
              ...
              {sortedRows.map((row) => {
                ...
                const isEditing = editingProduct?.id === row.id;
                ...
                return (
                  <tr
                    key={row.id}
                    className={`border-b transition-colors hover:bg-muted/30 cursor-pointer ${isEditing ? "bg-primary/5" : ""} ${isSelected ? "bg-primary/5" : ""}`}
                    onClick={() => setEditingProduct(isEditing ? null : row)}
                  >
                    ...
                    <td className="px-1 py-1">
                      {thumb
                        ? <img src={thumb} alt="" className="h-8 w-8 object-cover rounded shrink-0" />
                    ...
                    <td className="px-2 py-1.5">
                      <Badge variant="outline" className={`text-[10px] ${display.status === "active" ? "text-green-600 border-green-400" : display.status === "draft" ? "text-amber-600 border-amber-400" : "text-muted-foreground"}`}>
                        {display.status}
                      </Badge>
                    </td>
                    <td className="px-2 py-1.5 text-right tabular-nums">{fmt.price(row._price)}</td>
                    <td className="px-2 py-1.5 text-right tabular-nums text-muted-foreground">{fmt.price(row._cost_price)}</td>
                    <td className="px-2 py-1.5 text-right tabular-nums text-muted-foreground">{fmt.pct(row._gross_profit_percent)}</td>
                    <td className="px-2 py-1.5 text-right tabular-nums">{fmt.int(row._stock_on_hand)}</td>
                    <td className="px-2 py-1.5 text-right tabular-nums">{fmt.int(row._units_sold_12m)}</td>
                    <td className="px-2 py-1.5 text-muted-foreground font-mono text-[11px] whitespace-nowrap">{row._barcode ?? "—"}</td>
                    <td className="px-2 py-1.5 text-muted-foreground whitespace-nowrap">{display.product_type}</td>
                    <td className="px-2 py-1.5 text-muted-foreground max-w-[160px] truncate" title={tags}>{tags}</td>
                    <td className="px-2 py-1.5">
                      {row.has_edits
                        ? <Badge variant="outline" className="text-[10px] text-amber-600 border-amber-400">Edited</Badge>
                        : null}
                    </td>
        ...
        {selected.size > 0 && (
          <div className="shrink-0 border-t bg-background px-4 py-2 flex items-center gap-2 shadow-[0_-2px_8px_rgba(0,0,0,0.08)]">
            ...
            <Button
              size="sm"
              variant="outline"
              className="gap-1.5 h-7 text-amber-600 border-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950"
              disabled={bulkWorking}
              onClick={() => handleBulk({ status: "draft" }, "Deactivated")}
            >
            ...
            <Button
              ...
              disabled={bulkWorking}
              onClick={() => handleBulk({ status: "archived" }, "Archived")}
            >
            ...
            <Button
              size="sm"
              variant="outline"
              className="gap-1.5 h-7 text-green-600 border-green-400 hover:bg-green-50"
              disabled={bulkWorking}
              onClick={() => handleBulk({ status: "active" }, "Activated")}
            >
              Set Active
            </Button>
